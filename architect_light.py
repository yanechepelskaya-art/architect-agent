...
TELEGRAM_PROXIES = [
    ...
    {'https': 'https://149.154.167.40:443'},
    {'https': 'https://149.154.175.100:443'},
]
...
def process_updates():
    ...
    try:
        ...
        for upd in r.get("result", []):
            ...
            elif t in ["/lstm", "🧠 LSTM"]: send_tg("🧠 LSTM доступна только на полной версии (Mac).")
            elif t in ["/weights", "⚖ Веса"]: send_tg(show_prompt_weights())
            elif t in ["/ensemble", "🏛 Ансамбль"]: send_tg("🏛 Ансамбль доступен только на полной версии (Mac).")
            elif t in ["/transformer", "🔮 Transformer"]: send_tg("🔮 Transformer доступен только на полной версии (Mac).")
            elif t in ["/karma", "🙏 Карма"]: send_tg(show_karma())
            elif t in ["/cnn", "👁 CNN"]: send_tg(cnn_predict())
            elif t in ["/mega", "🏛 Мега-Ансамбль"]: send_tg("🏛 Мега-Ансамбль доступен только на полной версии (Mac).")
...
def compare_indices():
    try:
        # import yfinance  # отключено для лёгкой версии as yf
        result = []
...
def train_lstm_model():
    try:
        # import torch  # отключено для лёгкой версии
        # import torch  # отключено для лёгкой версии.nn as nn
        import numpy as np
        from sklearn.preprocessing import StandardScaler
...
def train_xgboost_model():
    try:
        # import xgboost  # отключено для лёгкой версии as xgb
        import numpy as np
        from sklearn.preprocessing import StandardScaler
...
def train_catboost_model():
    try:
        # from catboost  # отключено для лёгкой версии import CatBoostClassifier
        import numpy as np
        from sklearn.preprocessing import StandardScaler
...
def train_lightgbm_model():
    try:
        # from lightgbm  # отключено для лёгкой версии import LGBMClassifier
        import numpy as np
        from sklearn.preprocessing import StandardScaler
...
def get_crypto_news_sentiment():
    try:
        # import feedparser  # отключено для лёгкой версии
...
def ml_fake_breakout_detector():
    try:
        import numpy as np
        # from sklearn.ensemble import IsolationForest  # отключено для лёгкой версии
